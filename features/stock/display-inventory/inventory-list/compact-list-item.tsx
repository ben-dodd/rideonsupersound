...
import { getHoldQuantity, getItemSku, getLaybyQuantity } from 'lib/functions/displayInventory'
      ...
      <div className="w-full">
        <div className="flex w-full justify-between border-b border-gray-400">
          <div className="flex">
            ...
            <div className="font-bold text-md px-2">{`${item?.title || 'Untitled'}`}</div>
            <div className="text-md">{`${item?.artist || 'Untitled'}`}</div>
          </div>
          <div className={`${item?.needsRestock ? 'text-yellow-400' : 'text-red-400'} font-bold text-xl`}>
        ...
        <div className="flex">
          <div className="text-green-800">{`${item?.section ? `${item.section} / ` : ''}${item?.format} [${
            item?.isNew ? 'NEW' : item?.cond?.toUpperCase() || 'USED'
          }]`}</div>

          <div className="ml-4">{`${vendor ? `Selling for ${vendor?.name}` : ''}`}</div>
        </div>
      </div>
    </div>
  )
