...
import { getHoldQuantity, getImageSrc, getItemSku, getLaybyQuantity } from 'lib/functions/displayInventory'
...
export default function ListItem({ item }: ListItemProps) {
  ...
  const vendor = vendors?.find((vendor: VendorObject) => vendor?.id === item?.vendorId) || null
  ...
  return (
    <div className={`flex w-full mb-2 text-black ${item?.quantity < 1 ? 'bg-pink-200' : 'bg-gray-200'}`}>
      <div className="w-imageMed">
        <div className={`w-imageMed h-imageMed${item?.quantity < 1 ? ' opacity-50' : ''}`}>
          <img
      ...
      <div className="flex flex-col justify-between pl-2 w-full">
        <div>
          <div className="flex justify-between border-b items-center border-gray-400">
            <div>
              <div className="font-bold text-md">{`${item?.title || 'Untitled'}`}</div>
              <div className="text-md">{`${item?.artist || 'Untitled'}`}</div>
            </div>
            <div className={`${item?.needsRestock ? 'text-yellow-400' : 'text-red-400'} font-bold text-3xl`}>
              {item?.needsRestock ? 'PLEASE RESTOCK!' : itemQuantity < 1 ? 'OUT OF STOCK' : ''}
            </div>
          </div>
      ...
      <div className="self-center pl-8 hidden sm:inline">
        <Tooltip title="View and edit item details.">
          <button className="icon-button-large text-black hover:text-blue-500" onClick={null}>
