            ...
            return (
              ...
              >
                <div className="w-2/12 md:w-1/12 px-1">
                  <div className="hidden md:inline">{dayjs(pay?.date).format(dateSimple)}</div>
                  <div className="md:hidden">{dayjs(pay?.date).format(dateSimple)}</div>
                </div>
                <div className="w-1/6 px-1 text-right">${(pay?.amount / 100)?.toFixed(2)}</div>
                <div className="w-1/6 px-1 uppercase">{pay?.type}</div>
                <div className="w-4/12 md:w-5/12 px-1">
                  {pay?.type === VendorPaymentTypes?.DC || pay?.type === VendorPaymentTypes.Batch
                    ? pay?.bank_reference
                    : pay?.type === VendorPaymentTypes.Sale
                    ? storeCredits?.filter((s) => s?.vendor_payment_id === pay?.id)?.[0]?.item_list || pay?.note || ''
                    : pay?.note || ''}
                </div>
              </div>
            )
