    ...
    const res = await fetch(
      `/api/update-register?k=${process.env.NEXT_PUBLIC_SWR_API_KEY}`,
      {
        ...
        body: JSON.stringify({
          ...
          close_date: mysqlDate(dayjs.utc().format()),
        }),
      }
    )
    const json = await res.json()
    ...
    const res = await fetch(
      `/api/create-register?k=${process.env.NEXT_PUBLIC_SWR_API_KEY}`,
      {
        ...
        body: JSON.stringify({
          ...register,
          open_till_id: tillID,
          open_date: mysqlDate(dayjs.utc().format()),
        }),
      }
    )
    const json = await res.json()
...
export async function returnHoldToStock(
  ...
  updateHoldInDatabase({
    ...hold,
    date_removed_from_hold: mysqlDate(dayjs.utc().format()),
    ...
    const res = await fetch(
      `/api/create-stock-price?k=${process.env.NEXT_PUBLIC_SWR_API_KEY}`,
      {
        ...
        body: JSON.stringify({
          ...
          vendor_cut,
          total_sell,
          date_valid_from: mysqlDate(dayjs.utc().format()),
          note,
        }),
      }
    )
    ...
    const res = await fetch(
      `/api/create-stock-movement?k=${process.env.NEXT_PUBLIC_SWR_API_KEY}`,
      {
        ...
        body: JSON.stringify({
          ...
          sale_id,
          stocktake_id,
          date_moved: mysqlDate(dayjs.utc().format()),
        }),
      }
    )
    const json = await res.json()
...
export function processStocktake(
  ...
  updateStocktakeInDatabase({
    ...stocktake,
    date_closed: mysqlDate(dayjs.utc().format()),
    closed_by: clerk?.id,
  })
  updateStocktakeTemplateInDatabase({
    ...stocktakeTemplate,
    last_completed: mysqlDate(dayjs.utc().format()),
