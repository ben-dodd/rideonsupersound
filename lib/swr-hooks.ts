...
export function useRegisters(range) {
  const { data, error, mutate } = useSWR(
    `/api/get-registers?start_date=${range.startDate}&end_date=${range.endDate}`,
    fetcher
  )
  return {
    registers: data,
    isRegistersLoading: !error && !data,
    isRegistersError: error,
    mutateRegisters: mutate,
  }
}
